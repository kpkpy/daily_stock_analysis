...
class DashScopeWebSearchProvider(BaseSearchProvider):
    """
    阿里云 DashScope WebSearch MCP 搜索引擎
    ...
    API_ENDPOINT = "https://dashscope.aliyuncs.com/api/v1/mcps/WebSearch/mcp"

    def __init__(self, api_keys: List[str]):
        super().__init__(api_keys, "DashScope")
    ...
    def _do_search(
        ...
        try:
            ...
            if response.status_code != 200:
                error_msg = self._parse_error(response)
                logger.warning(f"[DashScope] Search failed: {error_msg}")
                return SearchResponse(
                    query=query,
                    results=[],
                    provider=self.name,
            ...
            try:
                ...
                logger.error(f"[DashScope] {error_msg}")
                return SearchResponse(
                    query=query,
                    results=[],
                    provider=self.name,
            ...
            logger.debug(f"[DashScope] Raw response: {data}")
...
class SearchService:
    ...
    def __init__(
        ...
        Args:
            ...
            dashscope_keys: DashScope WebSearch API Key 列表
        ...
        # 2. DashScope WebSearch（阿里云，中文优化）
        if dashscope_keys:
            self._providers.append(DashScopeWebSearchProvider(dashscope_keys))
            logger.info(f"已配置 DashScope 搜索，共 {len(dashscope_keys)} 个 API Key")
...
def get_search_service() -> SearchService:
    ...
    if _search_service is None:
        ...
        _search_service = SearchService(
            ...
            dashscope_keys=config.dashscope_api_keys,
