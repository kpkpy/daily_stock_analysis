...
class Config:
    ...
    dashscope_api_keys: List[str] = field(
        default_factory=list
    )  # DashScope WebSearch API Keys
    ...
    def _load_from_env(cls) -> "Config":
        ...
        dashscope_keys_str = os.getenv("DASHSCOPE_API_KEYS", "")
        dashscope_api_keys = [
            k.strip() for k in dashscope_keys_str.split(",") if k.strip()
        ]
    ...
    def _parse_llm_channels(cls, channels_str: str) -> List[Dict[str, Any]]:
        ...
        for raw_name in channels_str.split(","):
            ...
            models = [
                (f"openai/{m}" if "/" not in m and base_url else m) for m in models
            ]
